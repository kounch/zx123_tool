...
class NewEntryDialog:
    ...
    def __init__(self,
        ...
        main_frame: ttk.Frame = ttk.Frame(self.top, padding=10)
        main_frame.pack(fill='both')

        top_frame: ttk.Frame = ttk.Frame(main_frame, padding=10)
        top_frame.pack()
        extra_frame = ttk.Frame(main_frame, padding=10)
        extra_frame.pack(fill='x')
        bottom_frame: ttk.Frame = ttk.Frame(main_frame, padding=10)
        bottom_frame.pack(fill='x')

        name_label: ttk.Label = ttk.Label(top_frame, text=f'{str_name} Name:')
        name_label.pack(side='left')

        self.name_entry: ttk.Entry = ttk.Entry(top_frame, width=32)
        ...
        if self.b_rom:
            extra_label: ttk.Label = ttk.Label(extra_frame,
                                               text='ROM Settings:')
            extra_label.grid(column=0, row=1, sticky='w')
            self.extra_vars: list[Any] = []
            if b_alt:
                self.dict_rom_params['*'] = 'Rooted ROM'
            for index, key in enumerate(self.dict_rom_params):
                str_check: str = f'({key}) {self.dict_rom_params[key]}'
                self.extra_vars.append(tk.IntVar())
                check_1: ttk.Checkbutton = ttk.Checkbutton(
                    extra_frame,
                    text=str_check,
        ...
        ok_button: ttk.Button = ttk.Button(bottom_frame,
                                           text='OK',
                                           default='active',
                                           command=self.do_ok)
        ok_button.pack(fill='x', side='right')
        cancel_button: ttk.Button = ttk.Button(bottom_frame,
                                               text='Cancel',
...
class InfoWindow:
    ...
    def __init__(self: Any, parent: Any, str_name: str,
                 dict_data: dict[str, str | dict[str, Any]]):
        ...
        name_frame: ttk.Frame = ttk.Frame(main_frame, padding=2)
        name_frame.pack(fill='x')
        name_label: ttk.Label = ttk.Label(name_frame, text=f'Item: {str_name}')
        name_label.pack(side='left')

        kind_frame: ttk.Frame = ttk.Frame(main_frame, padding=2)
        kind_frame.pack(fill='x')
        kind_label: ttk.Label = ttk.Label(kind_frame,
                                          text=f'Kind: {dict_data["kind"]}')
        kind_label.pack(side='left')

        if dict_data['version']:
            version_frame: ttk.Frame = ttk.Frame(main_frame, padding=2)
            version_frame.pack(fill='x')
            version_label: ttk.Label = ttk.Label(
                version_frame, text=f'Version: {dict_data["version"]}')
            version_label.pack(side='left')

        dict_det = dict_data['detail']
        if dict_det:
            detail_frame: ttk.Frame = ttk.Frame(main_frame, padding=2)
            detail_frame.pack()
            detail_str: str = f'{dict_data["kind"]} family features:'
            detail_label: ttk.Label = ttk.Label(detail_frame, text=detail_str)
    ...
    def do_close(self, *_: Any):
        """Process OK Button"""
        self.top.destroy()

    def bind_keys(self, *_: Any):
        """Bind Menu Keys"""
...
class ROMPWindow:
    ...
    def __init__(self: Any, parent: Any, str_name: str, str_kind: str,
                 dict_roms: dict[str, Any], default_rom: int):
        self.parent = parent
        ...
        name_frame: ttk.Frame = ttk.Frame(main_frame, padding=2)
        name_frame.pack(fill='x')
        name_label: ttk.Label = ttk.Label(name_frame, text=f'File: {str_name}')
        name_label.pack(side='left')
        kind_label: ttk.Label = ttk.Label(name_frame, text=f'Kind: {str_kind}')
        kind_label.pack(side='right')

        self.roms_frame: ttk.Frame = ttk.Frame(main_frame, padding=5)
        self.roms_frame.pack()
        rom_label: ttk.Label = ttk.Label(self.roms_frame,
        ...
        #self.rom_menu = tk.Menu(self, tearoff=0)
    ...
    def rom_menu_popup(self: Any, *_: Any):
        """Unused"""
        print('No ROM Menu for ROMPack')
...
class ProgressWindow:
    ...
    def __init__(self: Any, parent: Any, str_title: str):
        self.parent = parent
        ...
        main_frame: ttk.Frame = ttk.Frame(self.top)
        main_frame.pack(fill='both')

        progress_frame: ttk.Frame = ttk.Frame(main_frame, padding=(25, 30))
        progress_frame.pack()
        progress_label: ttk.Label = ttk.Label(progress_frame,
                                              text='Updating...')
    ...
    def update(self: Any, str_message: str):
...
class PrefWindow:
    ...
    def __init__(self: Any, parent: Any):
        self.parent = parent
        ...
        settings_frame: ttk.Frame = ttk.Frame(main_frame, padding=10)
        settings_frame.pack(fill='x')

        self.dict_prefs: dict[str, str] = {
        ...
        bottom_frame: ttk.Frame = ttk.Frame(main_frame, padding=10)
        bottom_frame.pack(fill='x')
        restore_button: ttk.Button = ttk.Button(
            bottom_frame,
...
def center_on_parent(root: Any, window: Any):
    """
    ...
    parent: Any = root.nametowidget(window.winfo_parent())
